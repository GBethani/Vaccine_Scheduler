...
from shot.forms import VaccinationForm
from django.views import View
from django.http import HttpResponse, HttpResponseBadRequest
from shot.models import Vaccination
from shot.utils import generate_pdf
...
class ChooseCampaign(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(drug = self.kwargs["vaccine_id"])
...
def appointment_letter(request, vaccination_id):
    ...
    context = {
        ...
        "content": f"This is to inform that the {vaccination.campaign.drug.name} vaccination of Mr/Ms {vaccination.patient.get_full_name()} is scheduled on {vaccination.slot.date}",
    }
